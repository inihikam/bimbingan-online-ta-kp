                ...
                <table id="data-koor" class="table table-striped table-bordered table-responsive table-hover"
                    ...
                    @foreach ($dosen as $ds)
                        <tr>
                            ...
                            <td class="centered-column">{{ $ds->dosen->npp }}</td>
                            <td>{{ $ds->dosen->nama }}</td>
                            <td class="centered-column">{{ $ds->dosen->bidang_kajian }}</td>
                            <td class="centered-column">
                                <button class="btn btn-sm btn-secondary decrease-kuota" data-id="{{ $ds->status->id }}">
                                    -
                                </button>
                                &nbsp;<span class="kuota-mhs">{{ $ds->status->kuota }}</span>&nbsp;
                                <button class="btn btn-sm btn-success increase-kuota" data-id="{{ $ds->status->id }}">+
                                </button>
                            </td>
                            <td class="centered-column">{{ $ds->status->ajuan }}</td>
                            <td class="centered-column">{{ $ds->status->diterima }}</td>
                            <td class="centered-column">{{ $ds->status->sisa }}</td>
                            <td class="centered-column">{{ $ds->status->status }}</td>
                            <td class="text-center align-middle">
                                <div class="d-flex justify-content-center">
                                    <button class="btn btn-success btn-sm me-2" data-bs-toggle="modal"
                                            data-bs-target="#dialogDetailDsn" data-id="{{ $ds->id_dsn }}">
                                        <i class="fas fa-info-circle"></i>
                                    </button>
                                    <button class="btn btn-warning btn-sm me-2" data-bs-toggle="modal"
                                            data-bs-target="#dialogEditDosbingKoor" data-id="{{ $ds->id }}">
                                        <i class="far fa-edit"></i>
                                    </button>
                                    <button class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#dialogHapusKoor" data-id="{{ $ds->id }}">
